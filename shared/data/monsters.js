...
export const MONSTERS = {
  ...
  blue_slime: M({
    ...
    drops: [
      ...
      { id: 'scroll_mystery', chance: 0.003 },
    ],
  ...
  wild_boar: M({
    ...
    drops: [
      ...
      { id: 'box_weapon_1', chance: 0.0025 },
      { id: 'treasure_map', chance: 0.0015 },
      { id: 'scroll_mystery', chance: 0.005 },
    ],
  ...
  forest_spirit: M({
    ...
    drops: [
      ...
      { id: 'box_weapon_1', chance: 0.003 },
      { id: 'treasure_map', chance: 0.002 },
      { id: 'scroll_mystery', chance: 0.005 },
    ],
  ...
  alpha_wolf: M({
    ...
    drops: [
      ...
      // boxes, tradeable like any other: the reason to come back for it
      { id: 'box_weapon_1', chance: 0.1 },
      { id: 'treasure_map', chance: 0.08 },
      { id: 'scroll_mystery', chance: 0.15 },
  ...
  tree_guardian: M({
    ...
    drops: [
      ...
      { id: 'box_weapon_1', chance: 0.25 },
      { id: 'treasure_map', chance: 0.2 },
      { id: 'scroll_mystery', chance: 0.3 },
      // what every boss pays
...
/*
 * Amberwood's creatures (Monster Field 02, Lv 11-20): the Greenmire kinds
 * gone autumn - older, bigger, the colour of the leaves - drawn from their
 * cousins' sheets in other colours (sprite.tint) until they have their own.
 * Each keeps its cousin's moves and trades up its numbers; a kin drops its
 * cousin's material, a little more rarely, besides its own band's loot.
 */
const kin = (base, o) => {
  const b = MONSTERS[base];
  const { tint, scale, drops = [], ...rest } = o;
  const m = M({ ...b, ...rest, sprite: { ...b.sprite, tint, scale: (b.sprite.scale ?? 1) * (scale ?? 1) } });
  // (a boss's hoard and a summoned add's scrap are their own; the field loot is for the rest)
  m.drops = b.boss || !b.respawn ? drops : [...drops, ...potionDrops(m)];
  return m;
};
Object.assign(MONSTERS, {
  // in the meadows by the west bridge: the first thing over it, and slow
  amber_slime: kin('blue_slime', {
    id: 'amber_slime', name: 'Amber Slime', nameTh: 'สไลม์อำพัน', level: 11,
    hp: 300, atk: 30, def: 9, mdef: 6, hit: 78, flee: 64, exp: 50, jobExp: 29,
    element: 'earth', speed: 42, respawn: 12,
    tint: { hue: 185, sat: 1.15, light: 1.02 }, scale: 1.15,
    drops: [{ id: 'slime_jelly', chance: 0.5, qty: [1, 3] }],
    aurum: { chance: 0.4, min: 5, max: 9 },
  }),
  crimson_cap: kin('mushroom', {
    id: 'crimson_cap', name: 'Crimson Cap', nameTh: 'เห็ดใบแดง', level: 12,
    hp: 340, atk: 33, def: 11, mdef: 7, hit: 79, flee: 60, exp: 57, jobExp: 33,
    tint: { hue: 340, sat: 1.1, light: 0.92 }, scale: 1.12,
    drops: [{ id: 'mushroom_cap', chance: 0.45, qty: [1, 2] }, { id: 'herb', chance: 0.25 }],
    aurum: { chance: 0.4, min: 5, max: 10 },
  }),
  autumn_crawler: kin('caterpillar', {
    id: 'autumn_crawler', name: 'Autumn Crawler', nameTh: 'หนอนใบไม้ร่วง', level: 13,
    hp: 420, atk: 35, def: 14, mdef: 8, hit: 80, flee: 58, exp: 65, jobExp: 38,
    tint: { hue: 300, sat: 1.2, light: 1 }, scale: 1.15,
    drops: [{ id: 'caterpillar_leaf', chance: 0.45, qty: [1, 2] }, { id: 'soft_shell', chance: 0.25 }],
    aurum: { chance: 0.45, min: 6, max: 11 },
  }),
  // over the crossroads and the middle road, and they come to you
  amber_hornet: kin('forest_bee', {
    id: 'amber_hornet', name: 'Amber Hornet', nameTh: 'แตนอำพัน', level: 15,
    hp: 360, atk: 40, def: 10, mdef: 10, hit: 82, flee: 78, exp: 76, jobExp: 44,
    tint: { hue: 330, sat: 1.25, light: 0.95 }, scale: 1.15,
    drops: [{ id: 'honey', chance: 0.4 }, { id: 'bee_stinger', chance: 0.3 }],
    aurum: { chance: 0.45, min: 7, max: 12 },
  }),
  ember_boar: kin('wild_boar', {
    id: 'ember_boar', name: 'Ember Boar', nameTh: 'หมูป่าเปลวแดง', level: 16,
    hp: 520, atk: 44, def: 16, mdef: 8, hit: 83, flee: 64, exp: 86, jobExp: 50,
    element: 'fire', tint: { hue: 345, sat: 1.25, light: 0.9 }, scale: 1.18,
    charge: { ...MONSTERS.wild_boar.charge, power: 2.4, element: 'fire' },
    drops: [{ id: 'boar_meat', chance: 0.45, qty: [1, 2] }, { id: 'boar_hide', chance: 0.3 }, { id: 'boar_tusk', chance: 0.14 }],
    aurum: { chance: 0.55, min: 8, max: 14 },
  }),
  // round the ruined columns on the east side, in the dusk they are named for
  dusk_wisp: kin('forest_spirit', {
    id: 'dusk_wisp', name: 'Dusk Wisp', nameTh: 'ภูตสนธยา', level: 18,
    hp: 520, atk: 48, def: 12, mdef: 22, hit: 85, flee: 70, exp: 98, jobExp: 57,
    element: 'dark', tint: { hue: 200, sat: 1.1, light: 0.95 }, scale: 1.12,
    drops: [{ id: 'spirit_leaf', chance: 0.4 }, { id: 'forest_essence', chance: 0.2 }],
    aurum: { chance: 0.55, min: 9, max: 16 },
  }),
  // the mini boss, on the plateau up the north-east stairs
  ember_alpha: kin('alpha_wolf', {
    id: 'ember_alpha', name: 'Ember Alpha', nameTh: 'จ่าฝูงหมาป่าเพลิง', level: 19,
    hp: 3200, atk: 58, def: 20, mdef: 12, hit: 86, flee: 80, exp: 620, jobExp: 360,
    element: 'fire', tint: { hue: 170, sat: 1.3, light: 1.02 }, scale: 1.1,
    leap: { ...MONSTERS.alpha_wolf.leap, power: 2.2, element: 'fire' },
    howl: { ...MONSTERS.alpha_wolf.howl, element: 'fire' },
    enrage: { ...MONSTERS.alpha_wolf.enrage, say: 'จ่าฝูงหมาป่าเพลิงคลั่ง! โจมตีแรงและเร็วขึ้น' },
    drops: [...MONSTERS.alpha_wolf.drops],
    aurum: { chance: 1, min: 90, max: 160 },
  }),
  // the boss, in the ruined plaza: the old guardian of the woods, turned
  amber_warden: kin('tree_guardian', {
    id: 'amber_warden', name: 'Amber Warden', nameTh: 'ผู้พิทักษ์ป่าอำพัน', level: 20,
    hp: 9000, atk: 64, def: 24, mdef: 18, hit: 86, flee: 64, exp: 1800, jobExp: 1000,
    tint: { hue: 290, sat: 1.35, light: 1 }, scale: 1.05,
    calls: { ...MONSTERS.tree_guardian.calls, mob: 'amber_sapling',
      say: 'ผู้พิทักษ์ป่าอำพันเรียกต้นไม้มาฟื้นพลัง — ตัดต้นไม้ก่อน!' },
    enrage: { ...MONSTERS.tree_guardian.enrage, say: 'ผู้พิทักษ์ป่าอำพันโกรธเกรี้ยว!' },
    drops: [...MONSTERS.tree_guardian.drops],
    aurum: { chance: 1, min: 260, max: 420 },
  }),
  amber_sapling: kin('guardian_sapling', {
    id: 'amber_sapling', name: 'Amber Sapling', nameTh: 'ต้นกล้าอำพัน', level: 20,
    hp: 560, tint: { hue: 290, sat: 1.35, light: 1 },
    drops: [...MONSTERS.guardian_sapling.drops],
  }),
});
