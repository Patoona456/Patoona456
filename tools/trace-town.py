#!/usr/bin/env python3
"""Trace where a character may stand in Artaris.

    python3 tools/trace-town.py [overlay.png]

Two pictures of the same town, the same size and in register:
  assets/maps/source/artaris2/layout.png  the walk plan: paving, stairs and
                                            bridges between walls, lawns and water
  assets/maps/source/artaris3/full.png    the painted town on that plan, with
...
Writes shared/data/artaris-obstacles.js (90x60, '#' blocked).
"""
import os
import sys
...
PLAN = 'assets/maps/source/artaris2/layout.png'
...
def main(overlay=None):
    ...
    open_ = seen.copy()
    with open(os.path.join(ROOT, 'shared/data/artaris-obstacles.js'), 'w') as fp:
        ...
        fp.write('export const ARTARIS_OBSTACLES = [\n')
        for y in range(ROWS):
            fp.write("  '" + ''.join('.' if seen[y, x] else '#' for x in range(COLS)) + "',\n")
        fp.write('];\n')
    import base64
    bits = np.packbits((~seen_f).astype(np.uint8).ravel())
    with open(os.path.join(ROOT, 'shared/data/artaris-fine.js'), 'w') as fp:
        fp.write(f'// Generated by tools/trace-town.py - do not edit by hand.\n')
        fp.write('// Where a body may stand in Artaris, 4 world px a cell: one bit a cell,\n')
        fp.write('// row by row, 1 = shut (see shared/solids.js).\n')
        fp.write(f"export const ARTARIS_FINE = {{ cell: 4, cols: {fc}, rows: {fr}, bits: '{base64.b64encode(bits.tobytes()).decode()}' }};\n")
    if overlay:
        H, W = town.shape[:2]
        tw, th = W / COLS, H / ROWS
    ...
    print('artaris', int(seen.sum()), 'open tiles of', COLS * ROWS, '-', int(seen_f.sum()), 'open fine cells of', fc * fr)
