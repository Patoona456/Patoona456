...
def main(overlay=None):
    ...
    # The fine grid: 8 world px a cell, four to a tile each way. From the
    ...
    shut = cv2.morphologyEx(shut, cv2.MORPH_OPEN, np.ones((3, 3), np.uint8))        # kerbs go, walls stay
    ...
    with open(os.path.join(ROOT, 'shared/data/emberhold-fine.js'), 'w') as fp:
        ...
        fp.write('// Where a body may stand in Emberhold, 8 world px a cell: one bit a cell,\n')
        fp.write('// row by row, 1 = shut (see shared/solids.js).\n')
        fp.write(f"export const EMBERHOLD_FINE = {{ cell: 8, cols: {fc}, rows: {fr}, bits: '{base64.b64encode(bits.tobytes()).decode()}' }};\n")
    if overlay:
        H, W = town.shape[:2]
        tw, th = W / COLS, H / ROWS
