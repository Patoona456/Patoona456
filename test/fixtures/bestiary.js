// The bestiary as it stood before it was cleared for the new monster sheets.
//
// The live table comes back one sheet at a time, but the systems built around
// it - boss scripts, weekly lockouts, aggro by level gap, summoning scrolls -
// are code, and their tests should not wait on art. They borrow these.
import { MONSTERS, potionDrops } from '../../shared/data/monsters.js';
...
for (const m of Object.values(LEGACY_MONSTERS)) if (!m.drops?.length) m.drops = potionDrops(m);

/** Put the old monsters back beside the live ones, for this test process only. */
export function withLegacyBestiary() {
  for (const [id, m] of Object.entries(LEGACY_MONSTERS)) if (!MONSTERS[id]) MONSTERS[id] = m;
  return MONSTERS;
}

/**
 * Tests about how much the world holds - every band has a monster, every
 * element has a target, bosses pay the best swords - wait while the bestiary
 * is still being rebuilt: until something stands at the top of the game and a
 * boss exists to guard it.
 */
export const WAITING_FOR_MONSTERS = !(Object.values(MONSTERS).some((m) => m.boss)
  && Object.values(MONSTERS).some((m) => !m.summon && m.level >= 60))
  && 'the bestiary is only partly in: waiting for the rest of the monster sheets';
