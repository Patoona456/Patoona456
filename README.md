...
```
npm test          # ตรรกะทั้งหมด 91 เคส จบใน 8 วินาที ไม่ต้องติดตั้งอะไรเพิ่ม
npm run test:e2e  # เล่นจริงในเบราว์เซอร์ (ต้อง npm i ก่อน)
npm run test:all  # ทั้งสองชุด
npm run balance   # รายงานสมดุล: เวลาตีมอน ความคุ้มในแต่ละโซน เวลา 1→70
```
...
ถึงแคป เกณฑ์ทุกตัวถูกยืนยันซ้ำใน `test/balance.test.js` ด้วย คอมมิตที่แก้แต่ HP
มอนตัวเดียวจนทำเลเวลช่วงนั้นกลายเป็นกำแพงจะทำให้ CI ตก ไม่ใช่รอให้คนเล่นมาเจอเอง
