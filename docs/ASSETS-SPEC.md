...
| **Armor** | `armor` | มีแล้ว (ใส่ทับ Top ตามบอร์ด) |
| **Cloak / Cape** | `cloak` | มีแล้ว (ช่องหลัง แยกจากปีก) |
| **Glasses** | `glasses` | มีแล้ว |
| **Mask** | `mask` | มีแล้ว |
| **Scarf** | `scarf` | มีแล้ว |

ครบสิบห้าช่องแล้ว อยู่ที่ `shared/slots.js` ที่เดียว — ช่องหนึ่งบอกสามอย่าง:
เรียกว่าอะไร วาดจากไหน (`layer` = ชีต, `drawn` = รูปที่วาดด้วยโค้ด) และใส่ทับ
อะไรอยู่ เซิร์ฟเวอร์ ตุ๊กตาสวมใส่ และลำดับวาด อ่านจากตารางเดียวกัน

**เซฟเก่าไม่พัง** เพราะ `equipment` เป็นออบเจ็กต์ที่คีย์ด้วยชื่อช่อง ไม่ใช่ลิสต์
ตัวละครที่บันทึกตอนมีสิบช่องก็แค่ไม่มีคีย์ใหม่ห้าตัว มีเทสต์ยืนยันข้อนี้ไว้แล้ว
(ถ้าวันไหนมันกลายเป็นลิสต์ ทุกคนจะเอารองเท้าไปใส่หัว)

ศิลป์: **armor** ยืมชั้น `torso` ไปก่อน ส่วน **cloak scarf glasses mask**
วาดด้วยโค้ดที่ `client/js/apparel.js` แบบเดียวกับปีก — ช่องที่ใส่แล้วไม่มีอะไร
เปลี่ยนบนจอแย่กว่าช่องที่รูปเรียบง่าย และรอบที่แล้วเพิ่งเสียเวลาสองคอมมิตไป
กับอาวุธที่ชี้ไปยังศิลป์ที่วาดมันไม่ได้

**สมดุล**: ห้าช่องนี้จงใจให้พลังน้อย ถ้าผ้าคลุมให้ค่าเท่าอาวุธ ทุกคนก็ใส่ผ้าคลุม
ตัวเดียวกันหมด ช่องนั้นก็กลายเป็นภาษีไม่ใช่ทางเลือก เกราะซื้อ DEF แล้วจ่ายด้วย
ความเร็ว อีกสี่ช่องเล่นกับตัวเลขที่ตัดสินว่า*จะโดนไหม* ไม่ใช่*ตีแรงแค่ไหน*
และไม่มีชิ้นไหนมี ATK เลย — มีเทสต์บังคับไว้
